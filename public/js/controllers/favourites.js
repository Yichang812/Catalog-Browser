...
angular.module('ace')
.controller('Favourites', ['$scope', 'Global', 'UsersAPI', '$modal', '_', '$location', function ($scope, Global, UsersAPI, $modal, _, $location) {
	$scope.Global = Global;
	$scope.schematic = [];
	$scope.catalog = [];
	...
	$scope.getFavourites = function() {
		UsersAPI.me.query(function(response) {
			if(response)
			{
				...
				if (Global.user.userDefPrdt === 'AutoCAD MEP') {
					UsersAPI.getCatFav.query({product: Global.user.userDefPrdt}, function(catEntries) {
						$scope.catalog = catEntries;
					});
					UsersAPI.getFilters.query({product: Global.user.userDefPrdt}, function(catFilters) {
